...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const userSave = new User({ name, email, password, country, role });
  await userSave.save();
...
const followUser = asyncHandler(async (req, res) => {
  const followUser = await User.findById(req.params.profileId);
  await followUser.follow(req.user._id);

  const followingUser = await User.findById(req.user._id);
  await followingUser.addFollower(req.params.profileId);
