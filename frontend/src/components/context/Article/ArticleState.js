...
LIKE_ARTICLE_ERROR,
POST_COMMENT,
POST_COMMENT_ERROR,GET_ARTICLES,
GET_ARTICLES_ERROR,
} from "../types.js";
...
const {userInfo}=useContext(AuthContext)
  ...
  const postArticle=async(formData)=>{
   ...
   try{
    dispatch({ type: ARTICLE_LOADING_START });
       const {data}=await axios.post("/api/articles/createarticle",formData,config)
  ...
  const config = {
    ...
    try {
      // dispatch({ type: ARTICLE_LOADING_START });
      const { data } = await axios.post(`/api/articles/${articleId}/like`,config);
      console.log(data)
  ...
  try {
    ...
    const { data } = await axios.post(
      `/api/articles/${articleId}/comment`,text,
      config
    );
